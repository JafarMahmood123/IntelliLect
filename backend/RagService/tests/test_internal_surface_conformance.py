...
**The guard was a habit repeated per endpoint.** Eight routers, each with
...
away from ending, on a service whose routes are named `/api/search` and `/api/answer` rather than
`/api/internal/...`, which is precisely what a future `location /api/search` would look reasonable
for.

Duplicated in LiveAssistantService, following `test_migration_conformance.py`: the two services
share no test library and no virtualenv, and a rule that only one of them runs is a rule that half
the surface has.
"""
...
# This list started with a third entry, `/health/ready`, which this service does not serve — the
# both-directions check rejected it on the first run. Worth leaving a note about rather than just
# deleting: an exemption list is only worth having if a wrong entry is louder than a missing one.
