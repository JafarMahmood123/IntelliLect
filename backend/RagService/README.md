...
cp RagService/.env.example RagService/.env
# Ensure host Ollama is running (0.0.0.0:11434) with qwen3-embedding pulled.

docker-compose up --build rag-service rag-db
```

- `rag-db` runs `pgvector/pgvector:pg16` with a named volume and a
  healthcheck.
- `rag-service` waits for the DB to be healthy (`service_healthy`), then its
...
```bash
docker compose exec rag-service python scripts/embed_check.py
...
2. Watch `ingest_queue_depth` and `inflight_documents` (via `/metrics` or Prometheus),
   ...
   **container's memory** (`docker stats rag-service`).
