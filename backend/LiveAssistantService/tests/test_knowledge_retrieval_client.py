"""OFFLINE tests for KnowledgeRetrievalClient using httpx.MockTransport.

Asserts the exact request sent to KnowledgeService (URL / method / auth header /
...
def _settings() -> Settings:
    return Settings(knowledge_base_url="http://knowledge-service:8080", internal_api_secret="s3cret")


def _client(handler) -> KnowledgeRetrievalClient:
    return KnowledgeRetrievalClient(_settings(), transport=httpx.MockTransport(handler))
...
async def test_posts_to_search_with_secret_and_body_and_maps_results():
    ...
    assert str(request.url) == "http://knowledge-service:8080/api/search"
