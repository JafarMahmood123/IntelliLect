...
No model weights live in this container — every call is an HTTP request to host
Ollama (`POST /api/embed`). Used to embed transcript segments for LA-3 drift
...
class OllamaEmbeddingError(RuntimeError):
    """Raised when the local Ollama server cannot produce an embedding.

    The message is written to be actionable for an operator (is Ollama running? is
    the model pulled?) rather than surfacing a raw transport error.
    """
...
class OllamaEmbeddingProvider(EmbeddingProvider):
    ...
    async def embed_query(self, text: str) -> list[float]:
        ...
        return _l2_normalize(list(embeddings[0]))
