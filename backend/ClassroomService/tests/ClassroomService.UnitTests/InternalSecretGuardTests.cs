...
public sealed class InternalSecretGuardTests
{
    ...
    {
        // THE REGRESSION THIS FILE EXISTS FOR. The hand-rolled check this filter replaced returned
        // "authorized" when no secret was configured, so a missing or misspelled environment
        // variable silently opened every internal endpoint to anything that could reach the port.
    ...
    ///
    /// The check used to live inside each action, repeated twenty-three times across five
    /// controllers. Nothing anywhere would have noticed the twenty-fourth action that forgot it.
    /// This is what replaces that vigilance.
    /// </summary>
    [Fact]
    ...
    public void There_are_internal_controllers_to_check()
    {
        ...
        Assert.Equal(5, InternalControllers().Count);
    }
