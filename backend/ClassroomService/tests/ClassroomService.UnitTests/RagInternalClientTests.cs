...
public sealed class RagInternalClientTests
{
    private const string BaseUrl = "http://rag-service:8080/";
    private const string Secret = "test-internal-secret";

    private static RagInternalClient CreateClient(CapturingHttpMessageHandler handler)
    {
        var httpClient = new HttpClient(handler) { BaseAddress = new Uri(BaseUrl) };
        var options = Options.Create(new RagServiceOptions
        {
            BaseUrl = BaseUrl,
            InternalApiSecret = Secret,
            TimeoutSeconds = 10,
        });
        return new RagInternalClient(httpClient, options, NullLogger<RagInternalClient>.Instance);
    }
    ...
    public async Task NotifyFileUploaded_posts_ingest_with_body_and_secret_header()
    {
        ...
        Assert.Equal("http://rag-service:8080/api/internal/documents/ingest", request.Uri!.AbsoluteUri);
        Assert.Equal(Secret, request.SecretHeader);

        // Body must use RagService's camelCase aliases and the exact values.
    ...
    public async Task NotifyFileDeleted_sends_delete_to_document_url_with_secret_header()
    {
        ...
        Assert.Equal($"http://rag-service:8080/api/internal/documents/{fileId}", request.Uri!.AbsoluteUri);
        Assert.Equal(Secret, request.SecretHeader);
    }
    ...
    public async Task GetIndexingStatus_gets_status_url_with_secret_and_parses_status()
    {
        ...
        Assert.Equal($"http://rag-service:8080/api/internal/documents/{fileId}/status", request.Uri!.AbsoluteUri);
        Assert.Equal(Secret, request.SecretHeader);
    }
    ...
    public async Task GetAnswer_posts_question_with_scope_and_secret_and_parses_sources()
    {
        ...
        var handler = new CapturingHttpMessageHandler(() => new HttpResponseMessage(HttpStatusCode.OK)
        {
            // Response mirrors RagService: includes chunkId/score which must be ignored.
        ...
        var request = Assert.Single(handler.Requests);
        Assert.Equal(HttpMethod.Post, request.Method);
        Assert.Equal("http://rag-service:8080/api/answer", request.Uri!.AbsoluteUri);
        Assert.Equal(Secret, request.SecretHeader);
