...
public sealed class MigrationConformanceTests
{
    ...
    /// </summary>
    private static StreamingDbContext Context()
        => new(new DbContextOptionsBuilder<StreamingDbContext>()
    ...
    public void The_model_and_the_migrations_have_not_drifted()
    {
        ...
        Assert.False(
            ...
            + "Run: dotnet ef migrations add <Name> --project src/StreamingService.Infrastructure");
    }
    ...
    public void There_are_migrations_and_entities_to_check()
    {
        ...
        Assert.True(context.Model.GetEntityTypes().Count() >= 4, "the model looks empty");
    }

    private static List<Type> MigrationTypes()
        => typeof(StreamingDbContext).Assembly.GetTypes()
