import { useEffect, useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { Check, UserCog } from 'lucide-react';
import { useTranslation } from 'react-i18next';
import { useToast } from '../../../components/ui/ToastProvider';
import { searchUsers } from '../api/users';
import { useChangeClassroomTeacher } from '../hooks/useClassroomQueries';
import type { ClassroomAdminItem } from '../types';

interface ChangeTeacherDialogProps {
  // When set, the dialog transfers this classroom's ownership; null keeps it closed.
  classroom: ClassroomAdminItem | null;
  onClose: () => void;
  onChanged: (name: string) => void;
}

// Searchable picker over active teachers, backed by the user directory search (mirrors the create
// drawer's picker). Excludes the classroom's current teacher so the choice always means a change.
const TeacherPicker = ({
  selectedId,
  selectedLabel,
  excludeId,
  onSelect,
  error,
}: {
  selectedId: string;
  selectedLabel: string;
  excludeId: string;
  onSelect: (id: string, label: string) => void;
  error?: string;
}) => {
  const { t } = useTranslation('superAdmin');
  const [term, setTerm] = useState('');
  const [debounced, setDebounced] = useState('');
  const [open, setOpen] = useState(false);

  useEffect(() => {
    const id = window.setTimeout(() => setDebounced(term.trim()), 300);
    return () => window.clearTimeout(id);
  }, [term]);

  const { data, isFetching } = useQuery({
    queryKey: ['teacher-picker', debounced],
    queryFn: () =>
      searchUsers({ role: 'Teacher', status: 'Active', searchTerm: debounced, pageSize: 8 }),
    enabled: open,
  });

  const options = (data?.items ?? []).filter((teacher) => teacher.id !== excludeId);

  return (
    <div className="mb-2">
      <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
        {t('classrooms.changeTeacher.newTeacher')}
      </label>

      {selectedId && (
        <div className="mb-2 inline-flex items-center gap-2 rounded-md bg-violet-50 px-3 py-1.5 text-sm text-violet-700 dark:bg-violet-950/30 dark:text-violet-300">
          <Check size={14} />
          {selectedLabel}
        </div>
      )}

      <input
        type="text"
        value={term}
        onFocus={() => setOpen(true)}
        onChange={(e) => setTerm(e.target.value)}
        placeholder={t('classrooms.form.teacherPlaceholder')}
        className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-violet-500 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-100"
      />

      {open && debounced.length > 0 && (
        <ul className="mt-1 max-h-52 overflow-auto rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          {isFetching && (
            <li className="px-4 py-2 text-sm text-slate-400">{t('classrooms.form.searching')}</li>
          )}
          {!isFetching && options.length === 0 && (
            <li className="px-4 py-2 text-sm text-slate-400">{t('classrooms.form.noTeachers')}</li>
          )}
          {options.map((teacher) => (
            <li key={teacher.id}>
              <button
                type="button"
                onClick={() => {
                  onSelect(teacher.id, `${teacher.firstName} ${teacher.lastName}`);
                  setOpen(false);
                  setTerm('');
                }}
                className="flex w-full flex-col items-start px-4 py-2 text-left text-sm hover:bg-violet-50 dark:hover:bg-violet-950/20"
              >
                <span className="font-medium text-slate-800 dark:text-slate-200">
                  {teacher.firstName} {teacher.lastName}
                </span>
                <span className="text-xs text-slate-500">{teacher.email}</span>
              </button>
            </li>
          ))}
        </ul>
      )}

      {error && <span className="mt-1 block text-xs font-medium text-red-500">{error}</span>}
    </div>
  );
};

export const ChangeTeacherDialog = ({ classroom, onClose, onChanged }: ChangeTeacherDialogProps) => {
  const { t } = useTranslation('superAdmin');
  const { showToast } = useToast();

  const [teacherId, setTeacherId] = useState('');
  const [teacherLabel, setTeacherLabel] = useState('');
  const [reason, setReason] = useState('');
  const [errors, setErrors] = useState<{ teacher?: string; reason?: string }>({});
  const [serverError, setServerError] = useState('');

  const mutation = useChangeClassroomTeacher();

  useEffect(() => {
    if (classroom) {
      setTeacherId('');
      setTeacherLabel('');
      setReason('');
      setErrors({});
      setServerError('');
    }
  }, [classroom]);

  if (!classroom) return null;

  const submit = async () => {
    setServerError('');
    const next: typeof errors = {};
    // 4أ: a new teacher must be chosen; the picker already excludes the current owner (4ب).
    if (!teacherId) next.teacher = t('classrooms.changeTeacher.validation.teacherRequired');
    // 1أ: a reason is mandatory.
    if (reason.trim().length === 0) next.reason = t('classrooms.changeTeacher.validation.reasonRequired');
    setErrors(next);
    if (Object.keys(next).length > 0) return;

    try {
      await mutation.mutateAsync({
        id: classroom.id,
        data: { newTeacherId: teacherId, version: classroom.version, reason: reason.trim() },
      });
      showToast({
        type: 'success',
        title: t('classrooms.changeTeacher.successTitle'),
        message: t('classrooms.changeTeacher.success', { name: classroom.name, teacher: teacherLabel }),
      });
      onChanged(classroom.name);
      onClose();
    } catch (err: any) {
      const status = err?.response?.status;
      if (status === 404) {
        setServerError(t('classrooms.changeTeacher.notFound')); // 3أ
      } else if (status === 409) {
        setServerError(t('classrooms.changeTeacher.conflict')); // 3ب live session / concurrency
      } else if (status === 400) {
        setServerError(
          err?.response?.data?.detail || t('classrooms.changeTeacher.invalidTeacher'), // 4أ
        );
      } else {
        setServerError(err?.response?.data?.detail || t('classrooms.changeTeacher.fallbackError'));
      }
    }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-4 flex items-start gap-3">
          <span className="rounded-lg bg-violet-100 p-2 text-violet-600 dark:bg-violet-950/40 dark:text-violet-400">
            <UserCog size={20} />
          </span>
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">
              {t('classrooms.changeTeacher.title')}
            </h2>
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
              {t('classrooms.changeTeacher.description', { name: classroom.name })}
            </p>
          </div>
        </div>

        {/* Current owner — provides context for the transfer. */}
        <div className="mb-4">
          <label className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-300">
            {t('classrooms.changeTeacher.currentTeacher')}
          </label>
          <div className="rounded-lg bg-slate-100 px-4 py-2.5 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
            {classroom.teacherName || classroom.teacherEmail || t('classrooms.form.teacherUnknown')}
          </div>
        </div>

        {serverError && (
          <div className="mb-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
            {serverError}
          </div>
        )}

        <TeacherPicker
          selectedId={teacherId}
          selectedLabel={teacherLabel}
          excludeId={classroom.teacherId}
          onSelect={(id, label) => {
            setTeacherId(id);
            setTeacherLabel(label);
          }}
          error={errors.teacher}
        />

        <label className="mb-1 mt-2 block text-sm font-medium text-slate-700 dark:text-slate-300">
          {t('classrooms.changeTeacher.reasonLabel')}
        </label>
        <textarea
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          rows={3}
          placeholder={t('classrooms.changeTeacher.reasonPlaceholder')}
          className="w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-900 outline-none focus:border-violet-500 dark:border-slate-800 dark:bg-slate-950/50 dark:text-slate-100"
        />
        {errors.reason && <span className="mt-1 block text-xs font-medium text-red-500">{errors.reason}</span>}

        <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            type="button"
            onClick={onClose}
            disabled={mutation.isPending}
            className="inline-flex items-center justify-center rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-100 disabled:opacity-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900"
          >
            {t('common:buttons.cancel')}
          </button>
          <button
            type="button"
            onClick={submit}
            disabled={mutation.isPending}
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:from-violet-700 hover:to-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {mutation.isPending
              ? t('classrooms.changeTeacher.submitting')
              : t('classrooms.changeTeacher.submit')}
          </button>
        </div>
      </div>
    </div>
  );
};
