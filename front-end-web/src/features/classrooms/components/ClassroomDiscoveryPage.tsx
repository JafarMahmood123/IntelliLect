...
export const ClassroomDiscoveryPage = () => {
  ...
  const { data: allClassrooms, isLoading } = useDiscoveryClassrooms();
  const { data: enrolled } = useEnrolledClassrooms();
  const enrollMutation = useEnrollClassroom();

  const enrolledIds = new Set(enrolled?.map((c) => c.id));
  const availableClassrooms =
    allClassrooms?.filter((c) => !enrolledIds.has(c.id)) ?? [];
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center">
          ...
          <p className="text-slate-500">
            No new classrooms available to join right now.
          </p>
        </div>
      ) : (
              ...
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button
                  isLoading={
                    ...
                    enrollMutation.variables === classroom.id
                  }
